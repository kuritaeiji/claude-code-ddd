...
```go
// 例: エンティティのメソッドをそのまま呼んで状態と返り値を検証（復元はプリミティブ値で行う）
u := domain.ReconstructUser(uuid.NewString(), "alice@example.com", "Alice", domain.UserStatusActive)
...
| 担当者の活性整合性 | コマンド時のドメイン検証＋`UserDeactivated` イベントで担保。両者のすき間で稀に非活性ユーザーが担当に残るレースは、悲観ロックを避けて割り切る（詳細は domain.md「担当者の活性整合性に関する既知の制約」） |
