...
│   │   └── query/               # CQRSクエリ（読み込み）
│   │       └── mocks/           # TaskQueryRepository 等のモック実装（テストからのみ参照）
│   ├── controller/
│   │   └── ...                  # net/http ハンドラー
│   └── infrastructure/
...
| `backend/internal/usecase/query/`             | Usecase層             | 読み込み系クエリ（Q-01〜Q-02）を実装する。ドメインモデルを経由せず、DB から直接クエリ専用 DTO に詰める（JOIN 最適化のため）。`TaskQueryRepository` インターフェースと DTO もここで定義し、Infrastructure 層が実装する                                                                                                     |
| `backend/internal/usecase/query/mocks/`       | Usecase層（テスト用） | `usecase/query` に定義した `TaskQueryRepository` 等のインターフェースの testify/mock 実装。テストコードからのみ import し、プロダクションコードからは参照しない                                                                                                                                                           |
